...
// GenerateDefaultHWUID builds a deterministic host fingerprint hash.
func GenerateDefaultHWUID() string {
	host, _ := os.Hostname()
	raw := fmt.Sprintf("licensechain|go|%s|%s|%s", host, runtime.GOOS, runtime.GOARCH)
	return SHA256(raw)
}
